...
const posConvt2 = function (x, y, width) {
  [x, y] = [x, y].map(i => i + width * 0.5);
  return [x - 50, 50 - y].map((i, b) => i * options.map_size / 10 - b);
};
// ________________________________________________________________________________________
...
const MedRadar = function (ships, width, team) {
...
const yards = createYard();
game.custom.Yards = yards.map((i, index) => {
  let [x, y] = posConvt2(...i.position.slice(0, 2), 0.75);
  var [width, height] = [5893 * i.position[2] / 100, 5893 * i.position[3] / 100];
  return {
    position: i,
    ui: decorateBox(i),
    object: {
    ...
    team: null,
    area: i.position[2] * i.position[3],
    aliensSpawn: { x: x + width / 2, y: y + height / 2 },
    isAlienSpawn: false
  };
});
game.custom.Yards.map(i => game.setObject(i.object));
for (let i = 0; i < 200; i++) game.removeObject(i);
const radar_background = {
  id: 'radar_background',
  components: game.custom.Yards.map(i => i.ui)
};
// game.custom.data = [];
...
const tick = function (game) {
  if (game.step % 30 === 0) {
    ...
    if (!!game.ships[0]) {
      game.ships[0].setUIComponent(radar2);
      game.ships[0].setUIComponent(radar1);
      console.log(radar2);
      game.ships[0].setUIComponent(radar_background);
    }
  }
};
