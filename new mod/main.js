...
; (function () {
  game.radar = function (team) {
    function ComponentForShip(ship, width) {
      return {
        type: 'box', fill: tColors[ship.team],
        position: [ship.x, -ship.y].map((i, b) =>
          (i + game.options.map_size * 5 - b) / game.options.map_size * 10 - width * 0.5).concat([width, width])
      };
    }
    let result = game.ships.filter(ship => ship.team === team).map(ship => ComponentForShip(ship, width));
    ...
    let enemies = game.ships.filter(ship => ship.team !== team).map(ship => ComponentForShip(ship, width));
    ...
    return { id: radar, position: [25, 5, 85 * 0.5625, 85], component: result };
  }
})();