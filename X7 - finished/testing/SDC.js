...
Season 2 (v1.0.1) - Championship version
*/

// 1. qualify first before start (activate true) the game.
// 2. qualify player in the register.
// 3. pause if there are problems.
// 4. kick exploits.
// 5. when the game end, download the file.
// 6. 30 min for a whole match.
// 7. Figure out the areas
...
/* DEVELOPER'S TERMINAL COMMANDS
- activate <true/false>:      resume/pause the game.
- endgame <true/false>:       end the game.

- playerlist:                 show the detailed list of current ships in-game.
- kick <id> <reason>:         kick a player.

- disqualify <id> <reason>:   disqualify a player.
- qualify <id> <reason>:      qualify a player.
(Note that all newly joined players will be disqualified to prevent trolls).

- rankings:                   log the list of leaving ships + current ones.
(in the end, it will automatically logs this list).

- exportRankings:             same as above but download a file containing them instead.
- applyChanges:               apply changes after you edited the code.
*/
// Extended developer parts - Do not modify!

// devcodes
var dev_wait, locateShip = function (req, handler) {
...
}, toStatString = function (i, t) {
  return Math.max(i.wins, i.loses, i.draws) ? ((t + 1) + ". " + i.name + " (" + ["wins", "loses", "draws"].map(j => i[j] || 0).join("/") + ")") : "";
}, toRankString = function (stats, nLF) {
  nLF = nLF || ""
  let gt = (Array.isArray(stats) ? stats : []).map(toStatString).filter(i => i).join(nLF);
  return (gt ? nLF : "") + gt + (gt ? nLF : "");
}, ranktext = function (nLF) {
  let lf = "\n" + (nLF ? "\r\n\r" : "");
  return "LEAVING SHIPS:" + toRankString(game.custom.leaves, lf) + lf + "RANKINGS:" + toRankString(leaderboard, lf);
}
game.modding.commands.exportRankings = function () {
  var element = document.createElement('a');
  element.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(ranktext(true)));
  element.setAttribute('download', "rankings_" + new Date().toLocaleDateString().replace(/\//g, "-"));
...
game.modding.commands.activate = function (bool) {
  game.custom.dev_started = bool.trim().split(" ")[1] == "true";
  if (!game.custom.dev_started) dev_wait = game.step;
  else {
    if (game.custom.started) game.custom.started += ((game.step - dev_wait) || 0);
  }
  echo("The game is " + (game.custom.dev_started ? "resumed" : "paused"))
}
game.modding.commands.endgame = function (bool) {
  ended = bool.trim().split(" ")[1] == "true";
  if (ended) echo("The game will end after the current round has finished.");
  else echo("End game condition restored to default.")
}
game.modding.commands.playerlist = function () {
  echo("List of current players and their stats (W/L/D)")
  for (let ship of game.ships) {
    echo(ship.id + " : " + ship.name + " (" + ["wins", "loses", "draws"].map(i => getStat(ship, i)).join("/") + "). " + (ship.custom.qualified ? "" : "Not ") + "Qualified.")
  }
}
...
game.modding.commands.disqualify = function (req) {
  locateShip(req, function (ship, id, args) {
    ship.intermission({
      "You've been disqualified by the map host.": " ",
      "You will no longer have permissions to duel": " ",
      "Reason": args.slice(2, args.length).join(" ") || "No reason has been provided"
    });
    ship.custom.qualified = false;
    echo("Ship with id " + id + " was disqualified");
  })
}
game.modding.commands.qualify = function (req) {
  locateShip(req, function (ship, id, args) {
    ship.intermission({
      "You've been qualified by the map host.": " ",
      "You are ready to duel now.": " ",
      "Reason": args.slice(2, args.length).join(" ") || "No reason has been provided"
    });
    ship.custom.qualified = true;
    echo("Ship with id " + id + " was qualified");
  })
}
game.modding.commands.rankings = function () {
  echo(ranktext());
}
game.modding.commands.applyChanges = function () {
  game.ships.forEach(ship => (ship.custom.joined = false));
}
/* Modification changes
+ waiting/game_break check: game.custom.dev_started
+ ship.custom.joined: add join logging
+ a/b players ready: add ship.custom.qualified condition
. options.max_players: 30
. ready players: + !ship.custom.disqualified
. events button click with id "ready": + ship.custom.qualified
. leaderboard: +leaving players (game.custom.leaves)
. endgame: logging players to console
- ship_codes: only AS
- Map name
*/
...
var ship_infos = [A_Speedster_601].map(i => {
  let t = JSON.parse(i);
...
var modUtils = {
  ...
  var l = ld.map(i => i.id), d = leaderboard.map(i => i.id);
  for (let i of l) {
    let tx = d.indexOf(i);
    tx != -1 && d.splice(tx, 1);
  }
  if (d.length > 0) for (let i of d) {
    for (let x of leaderboard) {
      if (x.id === i) {
        echo(x.name + " (ID " + x.id + ") left the game.");
        if (Math.max(x.wins, x.loses, x.draws)) {
          if (Array.isArray(game.custom.leaves)) game.custom.leaves.push(x);
          else game.custom.leaves = [x];
        }
        break;
      }
    }
  ...
  for (let ship of game.ships) {
    if (!ship.custom.joined) {
      ...
      echo(ship.name + " joined the game. Player ID " + ship.id);
      ship.custom = {
        TpTimestamp: -1,
        pendingTp: -1,
        shipped: false,
...
var initialization = function (game) {
  ...
  if (!game.custom.started && game.step <= max_time || game.step - game.custom.started <= max_time || !game.custom.dev_started) {
    let pl = 2 - game.ships.length;
    if (pl <= 0 && game.custom.dev_started) {
    ...
    if (!ended) {
      ...
      check(game, true);
      updatescoreboard(game);
      game.modding.commands.exportRankings();
  ...
  if ((game.step - game.custom.started > toTick(game_duration + duel_duration / 2) && !round_break) || game.ships.length < 2 || !game.custom.dev_started) this.tick = waiting;
  else if (game.step % 30 === 0) {
    if (break_time >= 0) {
      if (game.step % 60 === 0) {
        let all_ships = game.ships.filter(i => i.custom.qualified), readies = all_ships.filter(i => i.custom.ready && !i.custom.spectate).length;
        setStats(game, readies + "/" + all_ships.length + " players ready");
      ...
      let ready = game.ships.filter(ship => ship.custom.qualified && ship.custom.ready && ship.alive && !ship.custom.spectate).sort(a => a.custom.wait ? -1 : 1), players = Math.trunc(Math.min(Math.pow(n, 2) - 1, ready.length / 2)), rlist = ready.map(ship => ship.id), lob = grids.map((v, i) => i).filter(i => i != lobby), index = 0, pairs = [...matches];
      ...
      if (ship.custom.currentNode === lobby) switch (id) {
        ...
        case "ready":
          if (ship.custom.qualified) {
            ...
            ship.custom.ready = false;
            addWarn(ship, "You are not qualified.")
          }
          break;
        case "spectate":
