...
/*
This isnt my mod (Senko). It is Wolfan's work.
I just stored here for future references 
*/
...
var powerAction = function (ship) {
  ...
  var bloodStealerInfos = {
    bloodstealer: `You drained a total of  ${(Math.trunc(ship.tier / 100) * 5) * ship.custom.batteries} shields and lost  ${(Math.trunc(ship.tier / 100) * 5) * ship.custom.batteries} crystals.`,
    bloodstealed: `You were drained of ${Math.trunc(((Math.trunc(ship.tier / 100) * 5) * ship.custom.batteries) / (game.ships.length - 1))
      } shields.`
  };
  ...
  switch (event.name) {
    case "ui_component_clicked":
      ...
      if (component == "learnMoreElectricityPath") {
        ship.instructorSays(space + `
          Damage players of ${((Math.trunc(ship.type / 100) * 10) / 2) * ship.custom.batteries} damages.
        `);
        ship.setUIComponent(explanationOK);
        ship.setUIComponent({ id: "pointsship", visible: false })
      }
      ...
      if (component == "learnMorebloodStealerPath") {
        ship.instructorSays(space + `
            Drain enemies’s shield (${game.ships.length - 1}) and lose ${Math.trunc(((Math.trunc(ship.tier / 100) * 5) * ship.custom.batteries) / (game.ships.length - 1))
